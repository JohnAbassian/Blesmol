	...
	public partial class Core : ServiceBase {
		...
		private void SendAlerts(String drive, String thresholdAmount, String thresholdUnit, DateTime eventTime, Boolean cleared) {
			...
			foreach (String email in c.EmailAddresses) {
				MailMessage message;
				if (cleared) {
					message = new MailMessage("alerts@reliant.org", email,
						"Disk space is now longer low on " + drive + " on "+ c.MachineName,
						"<p style='font-family:helvetica,arial,sans-serif;font-size:83%;'>Disk space is no longer low on " +
					...
					message = new MailMessage("alerts@reliant.org", email,
						"Warning! Disk space is low on " + drive + " on "+ c.MachineName,
